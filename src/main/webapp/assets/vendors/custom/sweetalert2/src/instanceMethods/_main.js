...
export function _main (userParams) {
  ...
  return new Promise((resolve) => {
    ...
    const getInputValue = () => {
      const input1 = this.getInput()
      if (!input) {
        return null
      }
      switch (innerParams.input) {
        case 'checkbox':
          return input1.checked ? 1 : 0
        case 'radio':
          return input1.checked ? input1.value : null
        case 'file':
          return input1.files.length ? input1.files[0] : null
        default:
          return innerParams.inputAutoTrim ? input1.value.trim() : input1.value
      }
    }
